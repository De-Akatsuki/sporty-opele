...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-[#f6f9fc] flex items-center justify-center">
      <div className="bg-white border border-[#e3e8ef] rounded-xl p-8 w-full max-w-sm shadow-sm">
        <div className="mb-6">
          <h1 className="text-xl font-semibold text-[#0a2540] tracking-tight">⚽ Sporty-Opele</h1>
          <p className="text-sm text-[#546e7a] mt-1">Sign in to your dashboard</p>
        </div>

        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-[#0a2540] mb-1">Email</label>
            <input
              type="email"
              value={email}
              onChange={e => setEmail(e.target.value)}
              required
              className="w-full border border-[#e3e8ef] rounded-lg px-3 py-2 text-sm focus:border-[#0570de] focus:ring-1 focus:ring-[#0570de] outline-none"
              placeholder="you@email.com"
            />
          </div>

          <div>
            <label className="block text-xs font-medium text-[#0a2540] mb-1">Password</label>
            <input
              type="password"
              value={password}
              onChange={e => setPassword(e.target.value)}
          ...
          {error && (
            <p className="text-xs text-[#df1b41]">{error}</p>
          )}

          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#0570de] text-white rounded-lg px-4 py-2 text-sm font-medium hover:bg-[#0560c9] disabled:opacity-50 transition-colors"
          >
